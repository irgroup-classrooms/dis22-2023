...
if __name__ == '__main__':
    ...
    core_nist = json.load(open("../data/core_nist.json", "r"))

    for query_type in ["QUERY_NO_EXAMPLE_NO_ATTRIBUTES", "QUERY_NO_EXAMPLE_NO_COMPLEXITY"]:
        
        for pers in Persona.list_register():
            persona = Persona.from_register(pers)
            query = Query()
            query.select_querie_type(query_type)
            ...
            partial_create_ds_line = partial(create_dataset_line, persona_name=persona.name, result_file=f"dataset_{query_type}.jsonl")
