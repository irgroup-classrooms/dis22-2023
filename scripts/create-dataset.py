...
def create_dataset_line(query, persona_name, result_file="dataset.jsonl"):
    content_dict  = send_query(query)
    save_result(result_file, (query, content_dict, persona_name))
...
def save_result(file, result: tuple[str, str, str]):
    query, queries, persona = result
...
if __name__ == '__main__':
    ...
    for pers in Persona.list_register():
        ...
        queries = [query.generate_query(title=nist["title"],
                                        desc=nist["desc"],
                                        n_queries=5)
